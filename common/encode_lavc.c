...
const struct m_sub_options encode_config = {
    .opts = (const m_option_t[]) {
        {"o", OPTF_STRING(file), .flags = CONF_NOCFG | CONF_PRE_PARSE | M_OPT_FILE},
        {"of", OPTF_STRING(format)},
        {"ofopts", OPTF_KEYVALUELIST(fopts), .flags = M_OPT_HAVE_HELP},
        {"ovc", OPTF_STRING(vcodec)},
        {"ovcopts", OPTF_KEYVALUELIST(vopts), .flags = M_OPT_HAVE_HELP},
        {"oac", OPTF_STRING(acodec)},
        {"oacopts", OPTF_KEYVALUELIST(aopts), .flags = M_OPT_HAVE_HELP},
        ...
        OPT_FLOATRANGE("oaoffset", aoffset, 0, -1000000.0, 1000000.0,
                       .deprecation_message = "--audio-delay (once unbroken)"),
        {"orawts", OPTF_FLAG(rawts)},
        {"ovfirst", OPTF_FLAG(video_first),
         .deprecation_message = "no replacement"},
        {"oafirst", OPTF_FLAG(audio_first),
         .deprecation_message = "no replacement"},
        {"ocopy-metadata", OPTF_FLAG(copy_metadata)},
        {"oset-metadata", OPTF_KEYVALUELIST(set_metadata)},
        {"oremove-metadata", OPTF_STRINGLIST(remove_metadata)},

        {"ocopyts", OPTF_REMOVED("ocopyts is now the default")},
        {"oneverdrop", OPTF_REMOVED("no replacement")},
        {"oharddup", OPTF_REMOVED("use --vf-add=fps=VALUE")},
        {"ofps", OPTF_REMOVED("no replacement (use --vf-add=fps=VALUE for CFR)")},
        {"oautofps", OPTF_REMOVED("no replacement")},
        {"omaxfps", OPTF_REMOVED("no replacement")},
        {0}
    },
