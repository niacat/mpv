...
const struct mp_cmd_def mp_cmds[] = {
    ...
    { "quit", cmd_quit, { {"code", OPTF_INT(v.i), .flags = MP_CMD_OPT_ARG} },
        .priv = &(const bool){0} },
    { "quit-watch-later", cmd_quit, { {"code", OPTF_INT(v.i), .flags = MP_CMD_OPT_ARG} },
        .priv = &(const bool){1} },
    { "stop", cmd_stop, },
    ...
    { "sub-step", cmd_sub_step_seek, { {"skip", OPTF_INT(v.i)} },
        .allow_auto_repeat = true, .priv = &(const bool){true} },
    { "sub-seek", cmd_sub_step_seek, { {"skip", OPTF_INT(v.i)} },
        .allow_auto_repeat = true, .priv = &(const bool){false} },
    { "print-text", cmd_print_text, { {"text", OPTF_STRING(v.s)} },
        .is_noisy = true, .allow_auto_repeat = true },
    { "show-text", cmd_show_text, { {"text", OPTF_STRING(v.s)},
                                    OPT_INT("duration", v.i, 0, OPTDEF_INT(-1)),
                                    {"level", OPTF_INT(v.i), .flags = MP_CMD_OPT_ARG}, },
        .is_noisy = true, .allow_auto_repeat = true},
    { "expand-text", cmd_expand_text, { {"text", OPTF_STRING(v.s)} },
        .is_noisy = true },
    { "expand-path", cmd_expand_path, { {"text", OPTF_STRING(v.s)} },
        .is_noisy = true },
    ...
    { "sub-add", cmd_track_add,
        {
            ...
            {"title", OPTF_STRING(v.s), .flags = MP_CMD_OPT_ARG},
            {"lang", OPTF_STRING(v.s), .flags = MP_CMD_OPT_ARG},
        },
    ...
    { "loadfile", cmd_loadfile,
        {
            ...
            OPT_CHOICE("flags", v.i, MP_CMD_OPT_ARG,
                       ({"replace", 0},
                        {"append", 1},
                        {"append-play", 2})),
            {"options", OPTF_KEYVALUELIST(v.str_list), .flags = MP_CMD_OPT_ARG},
        },
    },
    { "loadlist", cmd_loadlist, { {"url", OPTF_STRING(v.s)},
                                  OPT_CHOICE("flags", v.i, MP_CMD_OPT_ARG,
                                             ({"replace", 0}, {"append", 1})), },
        .spawn_thread = true,
        .can_abort = true,
    ...
    { "playlist-move", cmd_playlist_move,  { {"index1", OPTF_INT(v.i)},
                                             {"index2", OPTF_INT(v.i)}, }},
    { "run", cmd_run, { {"command", OPTF_STRING(v.s)},
                        {"args", OPTF_STRING(v.s)}, },
        .vararg = true,
    },
    { "subprocess", cmd_subprocess,
        {
            {"args", OPTF_STRINGLIST(v.str_list)},
            OPT_FLAG("playback_only", v.i, 0, OPTDEF_INT(1)),
            OPT_BYTE_SIZE("capture_size", v.i64, 0, 0, INT_MAX,
                          OPTDEF_INT64(64 * 1024 * 1024)),
            {"capture_stdout", OPTF_FLAG(v.i), .flags = MP_CMD_OPT_ARG},
            {"capture_stderr", OPTF_FLAG(v.i), .flags = MP_CMD_OPT_ARG},
        },
        .spawn_thread = true,
        .can_abort = true,
    },

    { "set", cmd_set, {{"name", OPTF_STRING(v.s)}, {"value", OPTF_STRING(v.s)}}},
    { "change-list", cmd_change_list, { {"name", OPTF_STRING(v.s)},
                                        {"operation", OPTF_STRING(v.s)},
                                        {"value", OPTF_STRING(v.s)} }},
    { "add", cmd_add_cycle, { {"name", OPTF_STRING(v.s)},
                              {"value", OPTF_DOUBLE(v.d), OPTDEF_DOUBLE(1)}, },
        .allow_auto_repeat = true,
        .scalable = true,
    },
    { "cycle", cmd_add_cycle, { {"name", OPTF_STRING(v.s)},
                                OPT_CYCLEDIR("value", v.d, 0, OPTDEF_DOUBLE(1)), },
        .allow_auto_repeat = true,
        .scalable = true,
        .priv = "",
    },
    { "multiply", cmd_multiply, { {"name", OPTF_STRING(v.s)},
                                  {"value", OPTF_DOUBLE(v.d)}},
        .allow_auto_repeat = true},

    { "cycle-values", cmd_cycle_values, { {"arg0", OPTF_STRING(v.s)},
                                          {"arg1", OPTF_STRING(v.s)},
                                          {"argN", OPTF_STRING(v.s)}, },
        .vararg = true},

    { "enable-section", cmd_enable_input_section,
        {
            {"name", OPTF_STRING(v.s)},
    ...
    { "af", cmd_filter, { {"operation", OPTF_STRING(v.s)},
                          {"value", OPTF_STRING(v.s)}, },
        .priv = &(const int){STREAM_AUDIO} },
    { "vf", cmd_filter, { {"operation", OPTF_STRING(v.s)},
                          {"value", OPTF_STRING(v.s)}, },
        .priv = &(const int){STREAM_VIDEO} },

    { "af-command", cmd_filter_command, { {"label", OPTF_STRING(v.s)},
                                          {"command", OPTF_STRING(v.s)},
                                          {"argument", OPTF_STRING(v.s)}, },
        .priv = &(const int){STREAM_AUDIO} },
    { "vf-command", cmd_filter_command, { {"label", OPTF_STRING(v.s)},
                                          {"command", OPTF_STRING(v.s)},
                                          {"argument", OPTF_STRING(v.s)}, },
        .priv = &(const int){STREAM_VIDEO} },
    ...
    { "script-binding", cmd_script_binding, { {"name", OPTF_STRING(v.s)} },
        .allow_auto_repeat = true, .on_updown = true},

    { "script-message", cmd_script_message, { {"args", OPTF_STRING(v.s)} },
        .vararg = true },
    { "script-message-to", cmd_script_message_to, { {"target", OPTF_STRING(v.s)},
                                                    {"args", OPTF_STRING(v.s)} },
        .vararg = true },

    { "overlay-add", cmd_overlay_add, { {"id", OPTF_INT(v.i)},
                                        {"x", OPTF_INT(v.i)},
                                        {"y", OPTF_INT(v.i)},
                                        {"file", OPTF_STRING(v.s)},
                                        {"offset", OPTF_INT(v.i)},
                                        {"fmt", OPTF_STRING(v.s)},
                                        {"w", OPTF_INT(v.i)},
                                        {"h", OPTF_INT(v.i)},
                                        {"stride", OPTF_INT(v.i)}, }},
    { "overlay-remove", cmd_overlay_remove, { {"id", OPTF_INT(v.i)} } },
    ...
    { "mouse", cmd_mouse, { {"x", OPTF_INT(v.i)},
                            {"y", OPTF_INT(v.i)},
                            OPT_INT("button", v.i, 0, OPTDEF_INT(-1)),
                            OPT_CHOICE("mode", v.i, MP_CMD_OPT_ARG,
                                       ({"single", 0}, {"double", 1})), }},
    { "keybind", cmd_key_bind, { {"name", OPTF_STRING(v.s)},
                                 {"cmd", OPTF_STRING(v.s)} }},
    { "keypress", cmd_key, { {"name", OPTF_STRING(v.s)} },
        .priv = &(const int){0}},
    { "keydown", cmd_key, { {"name", OPTF_STRING(v.s)} },
        .priv = &(const int){MP_KEY_STATE_DOWN}},
    { "keyup", cmd_key, { {"name", OPTF_STRING(v.s), .flags = MP_CMD_OPT_ARG} },
        .priv = &(const int){MP_KEY_STATE_UP}},

    { "apply-profile", cmd_apply_profile, {{"name", OPTF_STRING(v.s)}} },

    { "load-script", cmd_load_script, {{"filename", OPTF_STRING(v.s)}} },

    { "dump-cache", cmd_dump_cache, { {"start", OPTF_TIME(v.d), .min = MP_NOPTS_VALUE},
                                      {"end", OPTF_TIME(v.d), .min = MP_NOPTS_VALUE},
                                      {"filename", OPTF_STRING(v.s)} },
        .exec_async = true,
        .can_abort = true,
    },

    { "ab-loop-dump-cache", cmd_dump_cache_ab, { {"filename", OPTF_STRING(v.s)} },
        .exec_async = true,
        .can_abort = true,
    },
