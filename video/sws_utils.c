    ...
    .opts = (const m_option_t[]) {
        OPT_CHOICE("scaler", scaler, 0,
                   ({"fast-bilinear",   SWS_FAST_BILINEAR},
                    ...
                    {"lanczos",         SWS_LANCZOS},
                    {"spline",          SWS_SPLINE})),
        {"lgb", OPTF_FLOAT(lum_gblur), .min = 0, .max = 100.0},
        {"cgb", OPTF_FLOAT(chr_gblur), .min = 0, .max = 100.0},
        {"cvs", OPTF_INT(chr_vshift)},
        {"chs", OPTF_INT(chr_hshift)},
        {"ls", OPTF_FLOAT(lum_sharpen), .min = -100.0, .max = 100.0},
        {"cs", OPTF_FLOAT(chr_sharpen), .min = -100.0, .max = 100.0},
        {"fast", OPTF_FLAG(fast)},
        {"bitexact", OPTF_FLAG(bitexact)},
        {"allow-zimg", OPTF_FLAG(zimg)},
        {0}
    },
