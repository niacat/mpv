...
static const m_option_t vf_opts_fields[] = {
    {"fmt", OPTF_IMAGEFORMAT(fmt)},
    {"colormatrix", OPTF_CHOICE_C(colormatrix, mp_csp_names)},
    {"colorlevels", OPTF_CHOICE_C(colorlevels, mp_csp_levels_names)},
    {"primaries", OPTF_CHOICE_C(primaries, mp_csp_prim_names)},
    {"gamma", OPTF_CHOICE_C(gamma, mp_csp_trc_names)},
    {"sig-peak", OPTF_FLOAT(sig_peak)},
    {"light", OPTF_CHOICE_C(light, mp_csp_light_names)},
    {"chroma-location", OPTF_CHOICE_C(chroma_location, mp_chroma_names)},
    {"stereo-in", OPTF_CHOICE_C(stereo_in, mp_stereo3d_names)},
    {"rotate", OPTF_INT(rotate), .min = -1, .max = 359},
    {"w", OPTF_INT(w)},
    {"h", OPTF_INT(h)},
    {"dw", OPTF_INT(dw)},
    {"dh", OPTF_INT(dh)},
    {"dar", OPTF_DOUBLE(dar)},
    {"convert", OPTF_FLAG(convert)},
    {"outputlevels", OPTF_REMOVED("use the --video-output-levels global option")},
    {"peak", OPTF_REMOVED("use sig-peak instead (changed value scale!)")},
    {0}
};
