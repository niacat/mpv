...
const struct m_sub_options opengl_conf = {
    .opts = (const struct m_option[]) {
        {"opengl-glfinish", OPTF_FLAG(use_glfinish)},
        {"opengl-waitvsync", OPTF_FLAG(waitvsync)},
        {"opengl-swapinterval", OPTF_INT(swapinterval)},
        {"opengl-check-pattern-a", OPTF_INT(vsync_pattern[0])},
        {"opengl-check-pattern-b", OPTF_INT(vsync_pattern[1])},
        {"opengl-restrict", OPTF_INT(restrict_version)},
        {"opengl-es", OPTF_CHOICE(gles_mode,
         ({"auto", GLES_AUTO}, {"yes", GLES_YES}, {"no", GLES_NO}))},
        {"opengl-early-flush", OPTF_CHOICE(early_flush,
         ({"no", FLUSH_NO}, {"yes", FLUSH_YES}, {"auto", FLUSH_AUTO}))},

        {"opengl-debug", OPTF_REPLACED("gpu-debug")},
        {"opengl-sw", OPTF_REPLACED("gpu-sw")},
        {"opengl-vsync-fences", OPTF_REPLACED("swapchain-depth")},
        {"opengl-backend", OPTF_REPLACED("gpu-context")},
        {0},
    },
