...
const struct vo_driver video_out_vdpau = {
    ...
    .options = (const struct m_option []){
        {"deint", OPTF_INT(deint), .min = -4, .max = 4},
        {"chroma-deint", OPTF_FLAG(chroma_deint), OPTDEF_INT(1)},
        {"pullup", OPTF_FLAG(pullup)},
        {"denoise", OPTF_FLOAT(denoise), .min = 0, .max = 1},
        {"sharpen", OPTF_FLOAT(sharpen), .min = -1, .max = 1},
        {"hqscaling", OPTF_INT(hqscaling), .min = 0, .max = 9},
        {"fps", OPTF_FLOAT(user_fps)},
        {"composite-detect", OPTF_FLAG(composite_detect), OPTDEF_INT(1)},
        ...
        {"force-yuv", OPTF_FLAG(force_yuv)},
        {"queuetime_windowed", OPTF_REPLACED("queuetime-windowed")},
        {"queuetime_fs", OPTF_REPLACED("queuetime-fs")},
        {"output_surfaces", OPTF_REPLACED("output-surfaces")},
        {NULL},
    },
