...
const struct m_sub_options vulkan_conf = {
    .opts = (const struct m_option[]) {
        {"vulkan-device", OPTF_STRING_VALIDATE(device, vk_validate_dev)},
        ...
        {"vulkan-queue-count", OPTF_INT(queue_count), .min = 1, .max = 8},
        {"vulkan-async-transfer", OPTF_FLAG(async_transfer)},
        {"vulkan-async-compute", OPTF_FLAG(async_compute)},
        {0}
    },
