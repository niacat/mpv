...
const struct m_sub_options gl_video_conf = {
    .opts = (const m_option_t[]) {
        {"gpu-dumb-mode", OPTF_CHOICE(dumb_mode,
         ({"auto", 0}, {"yes", 1}, {"no", -1}))},
        {"gamma-factor", OPTF_FLOAT(gamma), .min = 0.1, .max = 2.0},
        {"gamma-auto", OPTF_FLAG(gamma_auto)},
        {"target-prim", OPTF_CHOICE_C(target_prim, mp_csp_prim_names)},
        {"target-trc", OPTF_CHOICE_C(target_trc, mp_csp_trc_names)},
        {"target-peak", OPTF_CHOICE(target_peak, ({"auto", 0})),
         .min = 10, .max = 10000},
        ...
        OPT_CHOICE("hdr-compute-peak", tone_map.compute_peak, 0,
                   ({"auto", 0},
                    {"yes", 1},
                    {"no", -1})),
        {"hdr-peak-decay-rate", OPTF_FLOAT(tone_map.decay_rate), .min = 1.0, .max = 1000.0},
        ...
        {"tone-mapping-max-boost", OPTF_FLOAT(tone_map.max_boost), .min = 1.0, .max = 10.0},
        {"tone-mapping-desaturate", OPTF_FLOAT(tone_map.desat)},
        OPT_FLOATRANGE("tone-mapping-desaturate-exponent",
                       tone_map.desat_exp, 0, 0.0, 20.0),
        {"gamut-warning", OPTF_FLAG(tone_map.gamut_warning)},
        {"opengl-pbo", OPTF_FLAG(pbo)},
        SCALER_OPTS("scale",  SCALER_SCALE),
        SCALER_OPTS("dscale", SCALER_DSCALE),
        SCALER_OPTS("cscale", SCALER_CSCALE),
        SCALER_OPTS("tscale", SCALER_TSCALE),
        {"scaler-lut-size", OPTF_INT(scaler_lut_size), .min = 4, .max = 10},
        {"scaler-resizes-only", OPTF_FLAG(scaler_resizes_only)},
        {"correct-downscaling", OPTF_FLAG(correct_downscaling)},
        {"linear-downscaling", OPTF_FLAG(linear_downscaling)},
        {"linear-upscaling", OPTF_FLAG(linear_upscaling)},
        {"sigmoid-upscaling", OPTF_FLAG(sigmoid_upscaling)},
        {"sigmoid-center", OPTF_FLOAT(sigmoid_center), .min = 0.0, .max = 1.0},
        {"sigmoid-slope", OPTF_FLOAT(sigmoid_slope), .min = 1.0, .max = 20.0},
        {"fbo-format", OPTF_STRING(fbo_format)},
        {"dither-depth", OPTF_CHOICE(dither_depth, ({"no", -1}, {"auto", 0})),
         .min = -1, .max = 16},
        OPT_CHOICE("dither", dither_algo, 0,
                   ({"fruit", DITHER_FRUIT},
                    ...
                    {"no", DITHER_NONE})),
        {"dither-size-fruit", OPTF_INT(dither_size), .min = 2, .max = 8},
        {"temporal-dither", OPTF_FLAG(temporal_dither)},
        {"temporal-dither-period", OPTF_INT(temporal_dither_period),
         .min = 1, .max = 128},
        {"error-diffusion",
         OPTF_STRING_VALIDATE(error_diffusion, validate_error_diffusion_opt)},
        OPT_CHOICE("alpha", alpha_mode, 0,
                   ({"no", ALPHA_NO},
                    {"yes", ALPHA_YES},
                    {"blend", ALPHA_BLEND},
                    {"blend-tiles", ALPHA_BLEND_TILES})),
        {"opengl-rectangle-textures", OPTF_FLAG(use_rectangle)},
        {"background", OPTF_COLOR(background)},
        {"interpolation", OPTF_FLAG(interpolation)},
        {"interpolation-threshold", OPTF_FLOAT(interpolation_threshold)},
        OPT_CHOICE("blend-subtitles", blend_subs, 0,
                   ({"no", BLEND_SUBS_NO},
                    {"yes", BLEND_SUBS_YES},
                    {"video", BLEND_SUBS_VIDEO})),
        {"glsl-shaders", OPTF_PATHLIST(user_shaders), .flags = M_OPT_FILE},
        {"glsl-shader", OPTF_CLI_ALIAS("glsl-shaders-append")},
        {"deband", OPTF_FLAG(deband)},
        {"deband", OPTF_SUBSTRUCT(deband_opts, deband_conf)},
        {"sharpen", OPTF_FLOAT(unsharp)},
        {"gpu-tex-pad-x", OPTF_INT(tex_pad_x), .min = 0, .max = 4096},
        {"gpu-tex-pad-y", OPTF_INT(tex_pad_y), .min = 0, .max = 4096},
        {"", OPTF_SUBSTRUCT(icc_opts, mp_icc_conf)},
        {"gpu-shader-cache-dir", OPTF_STRING(shader_cache_dir), .flags = M_OPT_FILE},
        {"gpu-hwdec-interop",
         OPTF_STRING_VALIDATE(hwdec_interop, ra_hwdec_validate_opt)},
        {"opengl-hwdec-interop", OPTF_REPLACED("gpu-hwdec-interop")},
        {"hwdec-preload", OPTF_REPLACED("opengl-hwdec-interop")},
        {"hdr-tone-mapping", OPTF_REPLACED("tone-mapping")},
        {"opengl-shaders", OPTF_REPLACED("glsl-shaders")},
        {"opengl-shader", OPTF_REPLACED("glsl-shader")},
        {"opengl-shader-cache-dir", OPTF_REPLACED("gpu-shader-cache-dir")},
        {"opengl-tex-pad-x", OPTF_REPLACED("gpu-tex-pad-x")},
        {"opengl-tex-pad-y", OPTF_REPLACED("gpu-tex-pad-y")},
        {"opengl-fbo-format", OPTF_REPLACED("fbo-format")},
        {"opengl-dumb-mode", OPTF_REPLACED("gpu-dumb-mode")},
        {"opengl-gamma", OPTF_REPLACED("gamma-factor")},
