...
const struct mp_user_filter_entry af_scaletempo = {
    .desc = {
        ...
        .options = (const struct m_option[]) {
            {"scale", OPTF_FLOAT(scale_nominal), .min = 0.01, .max = DBL_MAX},
            {"stride", OPTF_FLOAT(ms_stride), .min = 0.01, .max = DBL_MAX},
            {"overlap", OPTF_FLOAT(percent_overlap), .min = 0, .max = 1},
            {"search", OPTF_FLOAT(ms_search), .min = 0, .max = DBL_MAX},
