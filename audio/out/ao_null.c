...
const struct ao_driver audio_out_null = {
    ...
    .options = (const struct m_option[]) {
        ...
        {"buffer", OPTF_FLOAT(bufferlen), .min = 0, .max = 100},
        {"outburst", OPTF_INT(outburst), .min = 1, .max = 100000},
        {"speed", OPTF_FLOAT(speed), .min = 0, .max = 10000},
        {"latency", OPTF_FLOAT(latency_sec), .min = 0, .max = 100},
        {"broken-eof", OPTF_FLAG(broken_eof)},
        {"broken-delay", OPTF_FLAG(broken_delay)},
        {"channel-layouts", OPTF_CHANNELS(channel_layouts)},
        {"format", OPTF_AUDIOFORMAT(format)},
        {0}
    },
