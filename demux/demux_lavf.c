...
const struct m_sub_options demux_lavf_conf = {
    .opts = (const m_option_t[]) {
        {"demuxer-lavf-probesize", OPTF_INT(probesize), .min = 32, .max = INT_MAX},
        {"demuxer-lavf-probe-info", OPTF_CHOICE(probeinfo,
         ({"no", 0}, {"yes", 1}, {"auto", -1}, {"nostreams", -2}))},
        {"demuxer-lavf-format", OPTF_STRING(format)},
        {"demuxer-lavf-analyzeduration", OPTF_FLOAT(analyzeduration),
         .min = 0, .max = 3600},
        {"demuxer-lavf-buffersize", OPTF_INT(buffersize), 0,
         .min = 1, .max = 10 * 1024 * 1024, OPTDEF_INT(BIO_BUFFER_SIZE)},
        {"demuxer-lavf-allow-mimetype", OPTF_FLAG(allow_mimetype)},
        {"demuxer-lavf-probescore", OPTF_INT(probescore),
         .min = 1, .max = AVPROBE_SCORE_MAX},
        {"demuxer-lavf-hacks", OPTF_FLAG(hacks)},
        {"demuxer-lavf-o", OPTF_KEYVALUELIST(avopts)},
        {"sub-codepage", OPTF_STRING(sub_cp)},
        {"rtsp-transport", OPTF_CHOICE(rtsp_transport,
               ({"lavf", 0},
                {"udp", 1},
                {"tcp", 2},
                {"http", 3},
        ...
        {"demuxer-lavf-linearize-timestamps", OPTF_CHOICE(linearize_ts,
          ({"no", 0}, {"auto", -1}, {"yes", 1}))},
        {"demuxer-lavf-propagate-opts", OPTF_FLAG(propagate_opts)},
        {0}
    },
