...
const struct m_sub_options demux_conf = {
    .opts = (const struct m_option[]){
        {"cache", OPTF_CHOICE(enable_cache,
         ({"no", 0}, {"auto", -1}, {"yes", 1}))},
        {"cache-on-disk", OPTF_FLAG(disk_cache)},
        {"demuxer-readahead-secs", OPTF_DOUBLE(min_secs), .min = 0, .max = DBL_MAX},
        ...
        {"demuxer-donate-buffer", OPTF_FLAG(donate_fw)},
        {"force-seekable", OPTF_FLAG(force_seekable)},
        {"cache-secs", OPTF_DOUBLE(min_secs_cache), .min = 0, .max = DBL_MAX,
         .deprecation_message = "will use unlimited time"},
        {"access-references", OPTF_FLAG(access_references)},
        {"demuxer-seekable-cache", OPTF_CHOICE(seekable_cache,
         ({"auto", -1}, {"no", 0}, {"yes", 1}))},
        {"sub-create-cc-track", OPTF_FLAG(create_ccs)},
        {"stream-record", OPTF_STRING(record_file)},
        ...
        OPT_CHOICE_OR_INT("audio-backward-overlap", audio_back_preroll, 0, 0,
                          1024, ({"auto", -1})),
        {"video-backward-batch", OPTF_INT(back_batch[STREAM_VIDEO]),
         .min = 0, .max = 1024},
        {"audio-backward-batch", OPTF_INT(back_batch[STREAM_AUDIO]),
          .min = 0, .max = 1024},
        {"demuxer-backward-playback-step", OPTF_DOUBLE(back_seek_size),
         .min = 0, .max = DBL_MAX},
        {"metadata-codepage", OPTF_STRING(meta_cp)},
        {"demuxer-force-retry-on-eof", OPTF_FLAG(force_retry_eof),
