...
const struct m_sub_options stream_lavf_conf = {
    .opts = (const m_option_t[]) {
        {"stream-lavf-o", OPTF_KEYVALUELIST(avopts)},
        {"http-header-fields", OPTF_STRINGLIST(http_header_fields)},
        {"user-agent", OPTF_STRING(useragent)},
        {"referrer", OPTF_STRING(referrer)},
        {"cookies", OPTF_FLAG(cookies_enabled)},
        {"cookies-file", OPTF_STRING(cookies_file), .flags = M_OPT_FILE},
        {"tls-verify", OPTF_FLAG(tls_verify)},
        {"tls-ca-file", OPTF_STRING(tls_ca_file), .flags = M_OPT_FILE},
        {"tls-cert-file", OPTF_STRING(tls_cert_file), .flags = M_OPT_FILE},
        {"tls-key-file", OPTF_STRING(tls_key_file), .flags = M_OPT_FILE},
        {"network-timeout", OPTF_DOUBLE(timeout), .min = 0, .max = DBL_MAX},
        {"http-proxy", OPTF_STRING(http_proxy)},
        {0}
    },
